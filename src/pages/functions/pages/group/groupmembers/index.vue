<template>
  <div class="main-container">
    <loop-banner :texts="submitBanner"
                 :v-if="submitBanner.length" />
    <loop-banner v-if="allowTimeBanner.length"
                 :texts="allowTimeBanner"
                 theme="blue"
                 :hidable="false" />
    <div class="member-list">
      <i-swipeout v-for="(item, i) in membersinf"
        ...
        <div class="member-card"
          ...
          <div class="text-container">
            <div class="student-name">
              <span>{{ item.name }}</span>
              <color-tag :text="item.text"
                         :theme="item.theme" />
            </div>
            <div class="student-id">
              <span>{{ item.studentNo }}</span>
              <span v-for="timespan in item.myTimes"
                    :key="timespan">
                <color-tag :text="timespanMap[timespan]"
        ...
        <view slot="button"
              class="i-swipeout-demo-button-group">
          <view class="i-swipeout-demo-button delete-button"
                @click="beforeRemoveMember(item)">
            <i-icon size="32"
    ...
    <div v-if="isCaptain && !addblock && !groupSubmitted">
      ...
      <button hover-class="clicked"
              class="login-button"
              @click="displayAddBlock">添加新成员</button>
      <button hover-class="clicked"
              class="login-button"
              v-if="groupSubmitted === false"
              @click="beforeSubmitGroup">提交分组</button>
      <button hover-class="clicked"
              class="login-button"
              @click="beforeDisGroup">解散分组</button>
      <!-- <button
        hover-class="clicked"
        class="login-button"
        @click="testFunction"
    ...
    <button hover-class="clicked"
            class="login-button"
            @click="exitgroup"
            v-if="isCaptain==false && !groupSubmitted">退出分组</button>
  </div>
</template>
  ...
  methods: {
    refreshData () {
      ...
      const ret1 = GetSelfGroupInfoAPI(param).then(res => {
        ...
        if (!isSubmit) submitBanner.push('下拉可刷新信息')
    ...
    displayAddBlock () {
      ...
      let errorMessage
      if (membersinf.length >= 7) errorMessage = FEErrorMsg.CANNOT_MORE_THAN_5_MEMBERS
